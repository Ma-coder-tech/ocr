...
function cleanLabel(line, moneyTokens, percentTokens) {
    let label = line;
...
function benchmarkForProcessor(processorName) {
...
function statementPeriodFromText(text) {
    ...
    const monthYear = text.match(/\b(january|february|march|april|may|june|july|august|september|october|november|december|jan|feb|mar|apr|jun|jul|aug|sep|sept|oct|nov|dec)\b[^\n]{0,20}\b(20\d{2})\b/i);
    if (monthYear) {
...
function suspiciousReason(label) {
    const lower = label.toLowerCase();
...
function mergeAmountOnlyLines(lines) {
    const merged = [];
...
function pickVolume(candidates) {
    return [...candidates]
        .filter((candidate) => candidate.kind === "volume")
        .sort((left, right) => {
        const leftLabel = left.label.toLowerCase();
        const rightLabel = right.label.toLowerCase();
        const scoreVolumeLabel = (label) => {
            let score = 0;
            if (/total amount submitted/.test(label))
                score += 12;
            if (/total amount processed/.test(label))
                score += 11;
            if (/amounts? submitted/.test(label))
                score += 10;
            if (/amount funded|funded to your bank/.test(label))
                score += 8;
            if (/total volume|sales volume|gross sales|gross volume/.test(label))
                score += 8;
            if (/submitted|processed/.test(label))
                score += 5;
            if (/ytd|year to date|reportable/.test(label))
                score -= 20;
            if (/january|february|march|april|may|june|july|august|september|october|november|december/.test(label))
                score -= 6;
            return score;
        };
...
function pickTotalFees(candidates, totalVolume) {
    return [...candidates]
        ...
        .sort((left, right) => {
        const scoreFeeLabel = (label) => {
            const lower = label.toLowerCase();
            let score = 0;
            if (/total \(service charges.*interchange charges.*fees/.test(lower))
                score += 15;
            if (/fees charged/.test(lower))
                score += 12;
            if (/month end charge|less discount paid/.test(lower))
                score += 11;
            if (/^total fees$|total charges/.test(lower))
                score += 6;
            if (/total account fees|total transaction fees|total debit network fees|total service charges|total interchange charges/.test(lower))
                score += 4;
            if (/processing fees/.test(lower))
                score += 7;
            if (/fee|charge|discount/.test(lower))
                score += 4;
            return score;
        };
...
function buildFeeBreakdown(candidates, totalFees, totalFeeRow) {
...
export function refineTextOnlyPdfSummary(doc, baseSummary) {
    ...
    if (totalFees <= 0) {
        totalFees =
            Math.round(candidates
                ...
                .reduce((sum, candidate) => sum + candidate.amount, 0) * 100) / 100;
    }
    ...
    const benchmarkStatus = effectiveRate > benchmarkBase.upperRate ? "above" : effectiveRate < benchmarkBase.lowerRate ? "below" : "within";
    ...
    const dataQuality = [
        {
            level: "warning",
            message: "Numeric totals were recovered from searchable PDF text using heuristics. Verify the main totals against the original statement before acting on the report.",
        },
    ];
    if (!totalFeeRow) {
        dataQuality.push({
    ...
    const summary = {
        ...baseSummary,
        statementPeriod,
        executiveSummary: benchmarkStatus === "above"
