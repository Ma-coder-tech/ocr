...
function inferProcessor(text) {
    const v = text.toLowerCase();
    if (v.includes("paysafe"))
        return "Paysafe";
    if (v.includes("worldpay"))
        return "Worldpay";
    if (v.includes("payarc"))
        return "Payarc";
    if (v.includes("heartland") || v.includes("hps processing fee"))
        return "Heartland";
    if (v.includes("tsys"))
        return "TSYS";
    if (v.includes("clearent"))
        return "Clearent";
    if (v.includes("stripe"))
        return "Stripe";
    if (v.includes("square"))
        return "Square";
    if (v.includes("paypal"))
        return "PayPal";
    if (v.includes("adyen"))
        return "Adyen";
    if (v.includes("fiserv") || v.includes("first data") || v.includes("clover"))
        return "Fiserv/Clover";
    if (v.includes("elavon"))
        return "Elavon";
    if (v.includes("global payments"))
...
function parseDateish(value) {
    ...
    if (match) {
        ...
        if (map[month])
            return `${match[2]}-${map[month]}`;
    }
    return null;
}
function benchmarkForBusinessType(businessType) {
...
function inferPeriod(row, periodKeys) {
...
function createTextOnlyPdfSummary(doc, processorName, businessType) {
    ...
    const textWarnings = doc.extraction.reasons.map((message) => ({ level: "warning", message }));
    const dataQuality = [
        {
            level: "critical",
            message: "PDF preflight quality gate blocked numeric extraction. The tool is running text-only analysis to avoid misleading fee/volume outputs.",
        },
        ...textWarnings,
    ];
    const insights = [
    ...
    return {
        ...
        executiveSummary: "The uploaded PDF could not be parsed into structured numeric tables. To avoid inaccurate conclusions, numeric fee metrics are withheld and only qualitative checks are returned.",
        totalVolume: 0,
        totalFees: 0,
        effectiveRate: 0,
...
export function analyzeDocument(doc, businessType) {
    ...
    const feeCandidates = columnEntries
        ...
        .sort((a, b) => b.score - a.score);
    const selectedVolume = volumeCandidates.find((c) => c.stats.posSum > 0 && !includesAny(c.key.toLowerCase(), ["rate", "%", "ticket", "count"]));
    ...
    const feeBreakdown = [...feeBuckets.entries()]
        .map(([label, amount]) => ({
        label,
    ...
    const benchmark = {
        ...benchmarkBase,
        status: effectiveRate < benchmarkBase.lowerRate
            ? "below"
    ...
    for (const row of sampled) {
        ...
        if (selectedVolume && typeof row[selectedVolume.key] === "number") {
            current.volume += Math.max(0, row[selectedVolume.key]);
        }
    ...
    const suspiciousFees = feeBreakdown
        .filter((f) => f.amount > 0)
        .filter((f) => f.sharePct >= 6 ||
    ...
    const dynamicFields = [...numericColumns]
    ...
    const executiveSummary = benchmark.status === "above"
    ...
    const confidence = selectedVolume && selectedFeeColumns.length > 0 && dataQuality.every((d) => d.level !== "critical")
        ? "high"
