...
let checklistCachePromise = null;
...
function evaluateUniversalRule(element, doc, summary, text) {
    let status = "unknown";
...
function processorPatternForCheck(check) {
    const lower = check.toLowerCase();
    if (lower.includes("non-emv"))
        return /non[\s-]?emv/i;
    if (lower.includes("pci"))
        return /\bpci\b/i;
    if (lower.includes("monthly minimum"))
        return /monthly minimum/i;
    if (lower.includes("authorization fee"))
        return /authorization/i;
    if (lower.includes("customer intelligence suite"))
        return /customer intelligence suite/i;
    if (lower.includes("repricing") || lower.includes("fine print") || lower.includes("effective"))
        return /effective|billing change|terms/i;
    if (lower.includes("eirf") || lower.includes("non-qualified") || lower.includes("downgrade"))
        return /eirf|non-?qualified|downgrade/i;
    if (lower.includes("surcharge"))
        return /surcharge/i;
    return null;
}
function evaluateProcessorRule(id, title, doc, text) {
...
function evaluateCrossChecks(doc, summary, text, checks) {
    const resolveCrossCheckTag = (check) => {
        const lower = check.toLowerCase();
...
async function loadChecklists() {
    ...
    checklistCachePromise = (async () => {
        ...
        return {
            universal: JSON.parse(universalRaw),
...
export async function evaluateChecklistReport(doc, summary) {
    ...
    if (!detectedPack || processorDetection.confidence < 0.5) {
        ...
        processorResults = detectedPack.checks.map((check, index) => evaluateProcessorRule(`P${String(index + 1).padStart(3, "0")}`, check, doc, text));
    }
    const processorSpecific = {
