...
- AI-assisted fee classification is opt-in with `AI_FEE_CLASSIFICATION_ENABLED=true`; it sends only structured unresolved fee rows, not raw PDFs. `AI_FEE_CLASSIFICATION_PROVIDER=auto` tries Anthropic first and then OpenAI when `OPENAI_API_KEY` is configured.
- Anthropic fee classification defaults to `AI_FEE_CLASSIFICATION_MODEL=claude-opus-4-8`; OpenAI fallback defaults to `AI_FEE_CLASSIFICATION_OPENAI_MODEL=gpt-5.5`.
