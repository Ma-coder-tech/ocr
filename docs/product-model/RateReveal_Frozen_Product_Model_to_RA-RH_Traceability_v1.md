...
**Current scope:** Accepted RA–RH implementation plus accepted internal-analysis integration through the baseline above, and the bounded production deterministic-ingress package identified by `production_deterministic_ingress_v1`. That package authorizes only normal-upload ingress, a durable production-internal AnalysisRun, deterministic Fiserv-family/capability proof, and deterministic RB–RH execution/persistence. It does not authorize customer-report cutover, RF/RG production wiring, provider execution, public research, live AI, or autonomous research scheduling.
...
| RD | Economic charges, participant/control claims, statement-observed cost ledger and cost stack | `f447b9f` plus `production_deterministic_ingress_v1` | Fully implemented for accepted shadow V2 economic contract | Deterministically executed and snapshotted inside the production-internal AnalysisRun; no new ownership/control knowledge |
...
This connection is versioned as `canonical_analysis_run_v1` / `production_deterministic_ingress_v1` under `frozen_product_model_runtime_policy_v0_2`. Its production-internal authority is an enclosing execution and persistence boundary; it does not rewrite the RB–RH semantic manifests or make the legacy customer report consume V2 output.
...
| Deterministic RB → RC → RD → RE → RH is composed once in a shared core used by production and the existing evaluation harness | Schema v0.7 and Procedures A–AB; RH permission ceilings | `runtime/analysisRun.ts`; `evaluation/fiservEvaluationHarness.ts` | Production-internal deterministic execution; underlying semantic manifests remain unchanged | AnalysisRun, evaluation-harness, RB–RH regression, and Gold oracle tests |
| One run per production job is versioned, source-fingerprinted, idempotent, and persisted with per-stage status, hashes, limitations, work objectives, expected decision effect, and deterministic resource accounting | Runtime Policy Amendment v0.2 §§7–9 | `analysisRunTypes.ts`; `analysisRunStore.ts`; `src/db.ts` | Durable production-internal persistence; completed same-version retries reuse the run; failed retries clear stale authority; changed source under a job is rejected | `analysisRunStore.test.ts` |
...
| Gold remains an offline correctness oracle; RF production knowledge, RG, providers, public research, live AI, and adaptive/autonomous research are disabled in this slice | Gold v0.3 §§0, 18–20; Runtime Policy Amendment v0.2 and explicit package boundary | AnalysisRun manifest; no RF/RG/provider imports in the runtime core | Enforced by construction and manifest; no expansion of product authority | Build/full tests, Gold validation/tests/scope check, source-boundary inspection |
