...
- `statement-two` (`fsv-01-clover-full-oct`), a reusable full-layout deterministic profile. Its controlled planner now produces bounded public-research questions, so the existing deterministic-only runner fails closed before provider execution or Keychain access until a separately authorized provider package exists.
...
The full-layout preflight requires `fiserv_first_data_full_statement@1.0.0` admission. The versioned `fiserv_observation_subject_registry@2.0.0` inventories every nonzero fee observation, strips only controlled calculation suffixes, maps only explicitly registered product terms, and records every unmapped observation with a deterministic suppression reason. Exact aliases may aggregate into one subject only when the registry says they represent the same public terminology question. Fixture names, merchant identity, dates, amounts, observation counts, and empirical frequency never decide registry membership or question priority.

The current registry produces four bounded Fiserv historical processor-presentation subjects for both admitted full-layout control fixtures: multi-network assessment presentation, Discover Network Authorization Fee presentation, Discover Data Usage Fee presentation, and Visa Transaction Integrity Fee presentation. These claims do not represent Visa, Mastercard, Discover, or American Express network authority. The RF resolver runs before RG selection; an admitted answer suppresses research, and a conflict remains unresolved without AI arbitration. The accepted four-question ceiling and deterministic priority classes select the remaining eligible questions without amount ranking. Provider contexts contain only controlled public terminology, processor program, and period year. The persisted RG audit records raw and normalized inventory counts, opaque label fingerprints, mapped subjects, exact occurrence/evidence lineage, suppression reasons, RF disposition, selection, priority, materiality, required public-locator coverage, and current source-authority availability.

The exact `fiserv_cardpointe_card_organization_pass_through_fee_schedule_april_2022_v1` admission points only to Fiserv's April 2022 `Card Organization Pass-Through Fee Schedule` on `support.cardpointe.com`. Its pinned SHA-256 is `c6dd0ba734f25727f294c980fb25fdc320761f4a97bb512a1eac2ee75e69feba`, and its ceiling is `historical_processor_presentation_only`. The exact admission generates deterministic known-authority candidates, but URL, fingerprint, extraction, locator grounding, independent investigation, independent semantic verification, and production semantic validators remain mandatory. It cannot establish October 2024 continuity, Statement or Clover applicability, contract applicability, ownership, margin, markup, merchant pass-through, control, negotiability, avoidability, removability, Statement calculation, or savings.

The multi-network assessment subject requires independently verified locator-bound coverage for Visa, Mastercard, Discover, and American Express. A support set missing any required network is downgraded to partial support; one network locator cannot prove another network's presentation. Discover Network Authorization coverage requires the explicit Discover source label and cannot be grounded by a PayPal-only or other branded authorization label.

Because the Statement-2 profile was authorized as deterministic-only, any registered origin causes both readiness and normal execution to fail with `internal_live_deterministic_family_research_model_expansion_required`. It reports `Provider sends: 0` and `Keychain accesses: 0`. `provider-readiness` remains rejected. Enabling provider execution requires a separately designed, reviewed, and authorized package. A full-layout document that is not admitted—including a document that fails the required reconciliation controls—cannot reach observation planning at all.

The project Codex permission profile enables its managed network proxy and allows exactly `openrouter.ai`, `api.openai.com`, `merchants.fiserv.com`, and `support.cardpointe.com`. The runner separately verifies its committed host profile against the fixed provider endpoints and every production authority origin. Wildcards and registry-origin auto-allow remain disabled; adding another registry origin without explicit network-profile review therefore fails preflight.
