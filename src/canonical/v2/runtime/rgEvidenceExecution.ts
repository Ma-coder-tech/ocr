...
export async function executeDurableCanonicalRgEvidence(input: {
  ...
  for (const planned of selectedWork) {
    ...
    const currentClaimControlPlan = buildCanonicalClaimResearchControlPlan({
      atomicClaimId: admission.atomicClaimId,
      facet: admission.facet,
      researchAdmission: admission.researchAdmission,
      materiality: admission.materiality,
      decisionTier: admission.decisionTier,
      blockingPrerequisiteCodes: admission.decisionBasis.independentBlockingPrerequisiteCodes,
      knowledgeQueryPresent: admission.knowledgeQuery !== null,
      requiredSourceAuthorities: admission.requiredSourceAuthorities,
    });
    const recordedControlBelongsToCurrentClaim = latest.researchControl.plan.initialMarginalValueDecision.decisionId
      === currentClaimControlPlan.initialMarginalValueDecision.decisionId;
    const preSearchDecision = recordedControlBelongsToCurrentClaim
      ? latest.researchControl.marginalValueDecisions.at(-1) ?? latest.researchControl.plan.initialMarginalValueDecision
      : currentClaimControlPlan.initialMarginalValueDecision;
...
async function executeWorkItem(input: {
  ...
  for (const [index, candidate] of candidates.entries()) {
    ...
    const durableCandidateOutcome = canonicalCandidateResearchOutcome({ runId: input.runId, planHash: input.planHash,
    ...
    appendCandidateResearchOutcome(input.runId, durableCandidateOutcome);
    const reusableNegativeProof = canonicalReusableNegativeApplicabilityProof({
      ...
      outcome: durableCandidateOutcome,
    });
    ...
    candidateOutcomes.push(durableCandidateOutcome.outcomeClass);
    if (verified) break;
  }
  ...
  if (candidates.length >= MAX_CANDIDATES_PER_WORK_ITEM) {
    terminalizeWork(input.runId, input.workItem, "completed_unresolved", "unresolved",
      "rg_search_batch_completed_exact_semantic_support_insufficient", [], input.workerId);
