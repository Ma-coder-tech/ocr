...
export async function runFiservInternalAnalysisEvaluationV1(input: RunFiservInternalAnalysisInputV1): Promise<{
  ...
  const rgAudit: RgInternalAuditV1 = {
    ...
    searchAttempts: runtime.searchAttempts.map((attempt) => ({ ...attempt,
      queryTerms: [...attempt.queryTerms], candidateIds: [...attempt.candidateIds],
      candidateAudits: attempt.candidateAudits.map((candidate) => ({ ...candidate, reasonCodes: [...candidate.reasonCodes] })),
      reasonCodes: [...attempt.reasonCodes],
      providerMetadata: attempt.providerMetadata ? { ...attempt.providerMetadata } : null })),
    retrievalOutcomes: runtime.documents.map((document) => ({ ...document, locatorIds: [...document.locatorIds], reasonCodes: [...document.reasonCodes] })),
