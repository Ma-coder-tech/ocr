...
export function resolveFiservShortTemplateAdmission(input: {
  ...
  const proofs: Record<CanonicalEconomicsV2CapabilityId, string[]> = {
    ...
    funding_batches: [],
    settlement_adjustments: roleProof("settlement_adjustment", ["ADJUSTMENTS", "SUMMARY BY BATCH"]),
  ...
  return {
    ...
    sectionAdmissions: [
      ...
      { sourceSection: "SUMMARY BY BATCH", populationSemantics: ["canonical_net_submitted_card_volume", "settlement_adjustment_amount"], capabilityStatus: "supported", evidenceRefs: unique([...proofs.canonical_net_submitted_card_volume, ...proofs.settlement_adjustments]), limitations },
