...
export function resolveFiservRuntimeCapabilityAdmission(input: {
  ...
  const dynamicSupported: Partial<Record<CanonicalEconomicsV2CapabilityId, boolean>> = {
    ...
    settlement_adjustments: familyProven && separatedAdjustments
      && claimDependencyEvaluations.settlement_adjustments.status === "supported"
      && occurrenceRefsForRole("settlement_adjustment").length > 0,
    ...
    non_fee_financial_flow_exclusions: familyProven && Boolean(grossMinusRefundsProven
      || (separatedAdjustments && claimDependencyEvaluations.settlement_adjustments.status === "supported")
      || (separatedChargebacks && claimDependencyEvaluations.chargeback_financial_populations.status === "supported")),
  ...
  const capabilities = FISERV_CAPABILITY_IDS.map((capability) => {
    ...
    const proofEvidenceRefs = unique([...(capability === "processor_identity" ? protocolEvidenceRefs : []),
      ...
      ...(claimDependency?.proofEvidenceRefs ?? [])]);
    ...
    return { capability, status: supported ? "supported" as const : "unknown" as const,
      ...
      reasonCodes: supported ? unique(["exact_occurrence_population_and_claim_relevant_control_proven",
        ...(claimDependency?.reasonCodes ?? [])])
        : unique([familyProven ? "claim_specific_statement_proof_insufficient" : "supported_fiserv_protocol_identity_unresolved",
          ...(claimDependency?.reasonCodes ?? [])]),
