...
export function buildCanonicalFeeRollupAssessments(
  ledger: Pick<CanonicalFeeLedger, "controls" | "rows">,
): CanonicalFeeRollupAssessment[] {
  ...
  return grandControls.map((grand) => {
    const sections = printedControls.filter(
      (control) => control.basis === "section_control" && control.derivationGroupId === grand.derivationGroupId,
    );
    return assessGrandControl(grand, sections, eligibleRollupRowIds(ledger.rows));
  });
}
...
function assessGrandControl(
  ...
  const grandRows = sortedUnique(grand.coveredFeeRowIds);
  const sectionRows = sections.flatMap((section) => section.coveredFeeRowIds);
  ...
  const hasIncompleteControl = grandRows.length === 0 || sections.length === 0 || sections.some((section) => section.coveredFeeRowIds.length === 0);
  ...
  const reasonCodes = [
    ...
    ...(sections.some((section) => section.coveredFeeRowIds.length === 0) ? ["empty_section_control_membership"] : []),
...
function eligibleRollupRowIds(rows: CanonicalFeeLedger["rows"]): string[] {
  const excludedRoles = new Set(["section_subtotal", "fee_bucket_total", "statement_control_total", "informational_rate_row", "zero_dollar_reference_row", "duplicate_representation", "supporting_evidence_only"]);
  return rows.filter((row) => !excludedRoles.has(row.role)).map((row) => row.id).sort();
}
