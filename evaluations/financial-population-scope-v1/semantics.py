...
def role_for(table, column, closing_label):
    ...
    if table['kind'] == 'fees':
        # A structural schema's generated title is not printed financial evidence.
        context = normalized((table['title'] if table.get('titleRefs') else '') + ' ' + closing_label)
        fee_context = bool(re.search(r'\bfees?\b', context)) and not re.search(r'\b(?:basis|volume|rate|rates|estimated|estimate|projected|potential|proposed|forecast)\b', context)
        if not fee_context:
