...
- Blocker projection rule: Preserve every refusal internally. Project at most one comparison-unavailable record per distinct merchant-verifiable unlocker class; suppress equivalent records. Period-only and generic evidence-maintenance refusals use only the report-level limitation.
...
- Internal shadow decisions preserved: 54
- Merchant-visible shadow records before correction: 54
- Merchant-visible shadow records after correction: 0
...
- Historical case: Nov_2024_Statement.pdf retains candidate `runtime:doc_725263a9ef928ce4:feerow_e84ad0381fe7bdfe112a7701:component_dharma_retail_vmd_auth_v1` internally, while merchant projection uses only the report limitation. Repeated current-offer-versus-historical-period refusals do not create individual notices.
- Distinct useful blocker: merchant-specific approval or quote remains one VERIFY record, without naming an unavailable alternative or adding comparison arithmetic.
...
- historicalComparisonLeakDetected: 0
- webOrAiOperations: 0
