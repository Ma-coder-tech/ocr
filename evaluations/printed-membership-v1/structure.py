 ...
 for page in pages:
  ...
  for i,r in enumerate(rs):
   ...
   if active is None:
    ...
    if pending_previous and pending_previous['openEnd'] and not any(j>i and j<i+4 for j in headers) and heading(r) and n.startswith('fees') and pending_previous['kind']=='fees':
   ...
   if is_total and numeric_cells:
    ...
    if (not active['columns'][0]['printedLabel'] and norm(ar['label']).startswith('total')) or active['kind'] in ['funding','batch'] or norm(ar['label']).startswith(norm('Total '+active['title'])) or norm(ar['label'])=='total':
...
def validate_saved(packet,saved):
 """Recompute associations; hashes alone never confer admission."""
 return saved==assemble(packet)

if __name__=='__main__':
 import sys
 from pathlib import Path
 packet=json.loads(Path(sys.argv[1]).read_text());out=assemble(packet);Path(sys.argv[2]).write_text(json.dumps(out,indent=2))
